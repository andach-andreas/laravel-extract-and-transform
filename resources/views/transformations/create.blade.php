...
@section('content')
    ...
    <div class="bg-white shadow sm:rounded-lg p-6"
         ...
         x-init="$watch('sourceTable', () => fetchSourceColumns())">
        <form action="{{ route(config('extract-data.route_name_prefix', 'andach-leat.') . 'transformations.store') }}" method="POST" @submit="document.getElementById('configuration').value = generateJson()">
            @csrf
            ...
            <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2 mb-6">
                <div>
                    ...
                    <input type="text" name="name" id="name" x-model="name" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2 border">
                </div>
                <div>
            ...
            <div class="border-t pt-4">
                    ...
                    <template x-for="(col, index) in columns" :key="index">
                        <div class="flex items-start space-x-3 bg-gray-50 p-3 rounded border">
                            ...
                            <div class="w-1/4">
                                ...
                                <input type="text" x-model="col.dest" placeholder="e.g. first_name" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm sm:text-sm p-1 border">
                            </div>
                ...
                <div x-show="columns.length === 0" class="text-center py-8 text-gray-500 italic">
                    No columns mapped. Click "Add Column" to start.
                </div>
            </div>
