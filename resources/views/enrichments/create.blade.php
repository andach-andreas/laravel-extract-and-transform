...
@section('content')
    <div class="mb-6">
        <h1 class="text-2xl font-semibold text-gray-900">Create Enrichment Profile</h1>
    </div>
    ...
    <div class="bg-white shadow sm:rounded-lg p-6"
        ...
        <form action="{{ route(config('extract-data.route_name_prefix', 'andach-leat.') . 'enrichments.store') }}" method="POST">
            @csrf

            <div class="grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-2 mb-6">
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700">Name</label>
                    <input type="text" name="name" id="name" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2 border">
                </div>
                <div>
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="destination_table" id="destination_table" required class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2 border">
                </div>
            </div>
